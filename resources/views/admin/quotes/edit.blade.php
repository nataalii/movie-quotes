...
<div class="flex items-center justify-center h-screen">
    <form method="POST" action="{{ route('quote_update', $quote->id) }}" enctype="multipart/form-data" class="space-y-8 divide-y divide-gray-200 w-1/3 bg-white rounded-xl">
        @csrf
        @method('patch')
        <div class="space-y-8 divide-y divide-gray-200 sm:space-y-5 p-10">
          <div class="space-y-6 sm:space-y-5">
            <div>
              <h3 class="text-lg font-medium leading-6 text-gray-900"> Add Quote</h3>
              <p class="mt-1 max-w-2xl text-sm text-gray-500">Here you can add quotes from a movie</p>
            </div>
      
            <div class="space-y-6 sm:space-y-5">
                <div class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5">
                    <label for="email" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">Quote</label>
                    <div class="mt-1 sm:col-span-2 sm:mt ">
                      <input id="quote" name="quote" value="{{ $quote->quote }}" type="quote" autocomplete="quote" class="block bg-gray-100  p-3 w-full max-w-lg rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    </div>
                </div>
                @error('quote')
          ...
          <div class="space-y-6 pt-8 sm:space-y-5 sm:pt-10">
              ...
              <div class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:pt-5">
                <label for="movie_id" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">Movie</label>
