    ...
    <form method="POST" action="{{ route('quote.update', [app()->getLocale(), $quote->id]) }}" enctype="multipart/form-data" class="space-y-8 divide-y divide-gray-200 w-3/5 bg-white rounded-xl">
        ...
        <div class="space-y-8 divide-y divide-gray-200 sm:space-y-5 p-10">
          <div class="space-y-6 sm:space-y-5">
            <div>
              <h3 class="text-lg font-medium leading-6 text-gray-900"> {{ __('text.edit_quote') }}</h3>
              <p class="mt-1 max-w-2xl text-sm text-gray-500">{{ __('text.add_quotes') }}</p>
            </div>
      
            <div class="space-y-6 sm:space-y-5">
                <div class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5">
                  
                    <label for="email" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">{{ __('text.quote') }}</label>
                    <div class="mt-1 sm:col-span-2 sm:mt ">
                      <input id="quote" name="quote_ka" value="{{$quote->getTranslation('quote', 'ka')}}" type="quote"  placeholder="{{ __('text.quotes_in_ge') }}" class="block bg-gray-100  p-3 w-full max-w-lg rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                      <x-error name="quote_ka"/>

                      <input id="quote" name="quote_en" value="{{$quote->getTranslation('quote', 'en')}}" type="quote"  placeholder="{{ __('text.quotes_in_eng') }}" class="block bg-gray-100  p-3 w-full max-w-lg rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm mt-6">
              ...
              <div class="sm:grid sm:grid-cols-1 sm:items-start sm:gap-5 sm:border-t sm:border-gray-200 sm:pt-5">
                <label for="cover-photo" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">{{ __('text.upload_image') }}</label>
          ...
          <div class="space-y-6 pt-8 sm:space-y-5 sm:pt-10">
              ...
              <div class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:pt-5">
                <label for="movie_id" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">{{ __('text.movie') }}</label>
        ...
        <div class="pt-5 flex justify-end p-7">
            <button type="submit" class="ml-3 inline-flex justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">{{ __('text.update') }}</button>
        </div>
      </form>
  </div>
